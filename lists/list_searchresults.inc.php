...
class ListBlockCol_SearchResultModified extends ListBlockCol
{
    ...
    public function render_tr(&$r, $style = '')
    {
        if ($r instanceof SearchResult) {
            if (isset($r->item)) {
                if ($r->item->modified_by) {
                    if ($person = Person::getVisibleById($r->item->modified_by)) {
                        echo '<td><span class=date>' . renderDateHtml($r->item->modified) . '</span><br><span class="sub who">' . __('by') . ' ' . $person->getLink() . '</span></td>';
                        return;
                    }
                } else {
                    echo '<td><span class=date>' . renderDateHtml($r->item->modified) . '</span></td>';
                }
            }
            echo '<td></td>';
        } else {
