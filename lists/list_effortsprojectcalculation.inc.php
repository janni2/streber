...
class ListBlock_effortsProjectCalculation extends ListBlock
{
    ...
    */
    ...
    public function renderListHtml(&$project = null)
    {
        $this->render_header();
...
class ListBlockCol_EffortProjectName extends ListBlockCol
{
    ...
    public function render_tr(&$obj, $style = '')
    {
        ...
        $str = $PH->getLink('projView', $obj->name, ['prj' => $obj->id]);
...
class ListBlockCol_EffortProjectAmountHour extends ListBlockCol
{
    ...
    public function render_tr(&$obj, $style = '')
    {
        ...
        $sum_proj = Effort::getSumEfforts(['project' => $obj->id]);
        #}
...
class ListBlockCol_EffortProjectAmountSalary extends ListBlockCol
{
    ...
    public function render_tr(&$obj, $style = '')
    {
        ...
        if ($effort_people = Effort::getEffortPeople(['project' => $obj->id])) {
            foreach ($effort_people as $ep) {
                if ($person = Person::getVisibleById($ep->person)) {
                    ...
                    $sum_sal = Effort::getSumEfforts(['project' => $obj->id, 'person' => $person->id]);
                    #}
                    if ($sum_sal) {
                        $sum = (round($sum_sal / 60 / 60, 1) * 1.0);

                        if ($pp = $obj->getProjectPeople(['person_id' => $person->id])) {
...
class ListBlockCol_EffortProjectCalculation extends ListBlockCol
{
    ...
    public function render_tr(&$obj, $style = '')
    {
        ...
        if ($effort_tasks = Effort::getEffortTasks(['project' => $obj->id])) {
