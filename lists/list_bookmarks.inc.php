...
class ListBlockCol_ItemMonitored extends ListBlockCol
{
    ...
    public function render_tr(&$item, $style = '')
    {
        ...
        if ($notification_items = ItemPerson::getAll(['item' => $item->id, 'notify_on_change' => 1])) {
            echo '<td><img title="' . __('Notify on change') . '" src="' . getThemeFile('icons/monitored.png') . '"></td>';
        }
        ## notification only on unchanged ##
        elseif ($notification_items = ItemPerson::getAll(['item' => $item->id, 'notify_if_unchanged_min' => NOTIFY_1DAY])) {
            echo '<td><img title="' . __('Notify on change') . '" src="' . getThemeFile('icons/monitored.png') . '"></td>';
        } else {
            echo '<td>&nbsp;</td>';
        }
...
class ListBlockCol_ItemName extends ListBlockCol
{
    ...
    public function render_tr(&$item, $style = '')
    {
        ...
        if ($type = $item->type) {
            ...
            echo "<td class='nowrap'><span $isDone><a href='$str_url'>$str_name</a> $str_addon</span>";
            if ($html_details) {
...
class ListBlockCol_ItemComment extends ListBlockCol
{
    ...
    public function render_tr(&$item, $style = '')
    {
        ...
        if ($bookmark_items[0]->comment) {
            echo '<td><span>' . $bookmark_items[0]->comment . '</span></td>';
        } else {
            echo '<td>-</td>';
        }
    }
        ...
        if ($ip = ItemPerson::getAll(['item' => $item->id, 'notify_if_unchanged_min' => NOTIFY_1DAY])) {
            ...
            if ($notify_date != '0000-00-00 00:00:00') {
                ...
                if ($send_date > $current_date) {
                    ...
                    echo '<td>' . sprintf(__('in %s day(s)'), $days) . '</td>';
                } else {
                    $days = round(($current_date - $send_date) / 60 / 60 / 24);
                    echo '<td>' . sprintf(__('since %s day(s)'), $days) . '</td>';
                }
            } else {
                echo '<td>&nbsp;</td>';
            }
