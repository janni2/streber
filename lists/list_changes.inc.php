...
class ListBlockCol_ChangesDate extends ListBlockCol
{
    ...
    public function render_tr(&$change_line, $style = '')
    {
        if ($change_line instanceof ChangeLine) {
            if ($change_line->person_by) {
                if ($person = Person::getVisibleById($change_line->person_by)) {
                    echo '<td><span class=date>' . renderDateHtml($change_line->timestamp) . '</span><br><span class="sub who">' . __('by') . ' ' . $person->getLink() . '</span></td>';
                    return;
                }
            }
            echo '<td><span class=date>' . renderDateHtml($change_line->timestamp) . '</span></td>';
        } else {
...
class ListBlockCol_ChangesWhat extends ListBlockCol
{
    ...
    public function render_tr(&$change_line, $style = '')
    {
        ...
        echo '<td><span class=nowrap>' . $str_what . '</span><br><span class="sub who">' . $html_assignment . '</span></td>';
    }
}
        ...
        if ($change_line->task_html) {
            ...
            if ($change_line->item->type == ITEM_TASK) {
                ...
                $str_details = $change_line->html_details
                         ...
                         : '';
            } elseif ($change_line->item->type == ITEM_FILE) {
                $str_item = $PH->getLink('fileView', $change_line->item->name, ['file' => $change_line->item->id]);

                $str_details = $change_line->item->renderLocation();
            }
        } elseif ($change_line->item_id) {
        }

        echo '<td>' . $str_item . '<br><span class="sub">' . $str_details . '</span></td>';
    }
}
...
class ListBlockCol_ChangesDatePerson extends ListBlockCol
{
    ...
    public function render_tr(&$item, $style = '')
    {
        ...
        } elseif ($date_modified > $date_created) {
        ...
        echo '<td><span class=date>' . $renderTimestampHtml($date_modified) . '</span><span class=sub>' . $person->getLink() . '</span></td>';
    }
}
...
class ListBlockCol_AllChangesItemName extends ListBlockCol
{
    ...
    public function render_tr(&$obj, $style = '')
    {
        ...
        echo "<td><a href='$str_url'>$str_name</a><span class='sub who'> $str_addon</span></td>";
    }
}
