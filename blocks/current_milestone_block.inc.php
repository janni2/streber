...
class CurrentMilestoneBlock extends PageBlock
{
    ...
    */
    private function render_milestoneGraph($milestone_task_summary)
    {
        ...
        echo $PH->getLink(
            'projViewTasks',
            sprintf(__('%s completed task'), $milestone_task_summary['num_closed']),
            [
                        'prj' => $this->project->id,
             ...
             . $PH->getLink(
                 'projViewTasks',
                 sprintf(__('%s open'), $milestone_task_summary['num_open'] - $milestone_task_summary['num_need_approval']),
                 [
                        'prj' => $this->project->id,
                        'for_milestone' => $this->current_milestone->id,
        ...
        if ($milestone_task_summary['num_need_approval']) {
            echo $PH->getLink(
                'projViewTasks',
                $milestone_task_summary['num_need_approval'] < 2
