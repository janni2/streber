...
class SearchResult extends BaseObject
{
    ...
    public static function RateItem($item)
    {
        ...
        if ($item->project && $project = Project::getById($item->project)) {
            ...
            } elseif ($project->status <= STATUS_OPEN) {
    ...
    public static function GetExtract($item, $search)
    {
        $strings = [];
        ...
        if (isset($item->description)) {
            ...
            if (preg_match("/(.{0,90}$s.{0,120})/i", $diz, $matches)) {
                $extract = $matches[1];
            } elseif (preg_match('/(.{0,90}' . $strings[0] . '.{0,120})/i', $diz, $matches)) {
                $extract = $matches[1];
            }
        }
        $foo = asHtml($extract);
    ...
    public static function RateTitle($item, $search)
    {
        $rate = 1;
        ...
        if (isset($item->name)) {
            foreach ($strings as $s) {
                ...
                } elseif (preg_match("/\A$s/i", $item->name)) {
                    $rate *= RATE_IN_NAME_START;
                } elseif (preg_match("/\b$s\b/i", $item->name)) {
        ...
        if (isset($item->short)) {
            foreach ($strings as $s) {
                if (preg_match("/\A$s\b/i", $item->short)) {
                    $rate *= RATE_MATCHES_NAME;
                } elseif (preg_match("/\A$s/i", $item->short)) {
                    $rate *= RATE_IN_NAME_START;
                } elseif (preg_match("/\b$s\b/i", $item->short)) {
                    $rate *= RATE_MATCHES_NAME_WORD;
                }
            }
        }
    ...
    public static function getForQuery($search_query, $project = null)
    {
        $count_overall = 0;
        $results = [];
        global $PH;
            ...
            if ($tasks) {
                foreach ($tasks as $task) {
                    ...
                    if ($task->status > STATUS_COMPLETED) {
                        $rate *= RATE_TASK_STATUS_CLOSED;
                    } elseif ($task->status >= STATUS_COMPLETED) {
                        $rate *= RATE_TASK_STATUS_COMPLETED;
                    }
