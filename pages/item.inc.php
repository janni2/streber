    ...
    ### list changes ###
    {
        ...
        if (count($versions) == 1) {
            ...
            for ($i = 0; $i < count($versions) - 1; $i++) {
                ...
                } elseif ($v->version_number > $version_left->version_number) {
            ...
            for ($i = 1; $i < count($versions); $i++) {
                ...
                } elseif ($v->version_number > $version_right->version_number) {
            ...
            foreach ($new_field_values as $field_name => $value) {
                ...
                if ($field_type == 'FieldText') {
                    ...
                    echo render_changes($old_value, $new_value);
                } elseif ($field_type == 'FieldInternal') {
                    ...
                    echo render_changes($old_value, $new_value);
                } elseif ($field_type == 'FieldPercentage') {
                    echo render_changes($old_value, $new_value);
                } elseif ($field_type == 'FieldInt') {
                    echo render_changes($old_value, $new_value);
                } elseif ($field_type == 'FieldString') {
                    echo render_changes($old_value, $new_value);
                } elseif ($field_type == 'FieldDate') {
                    echo render_changes(renderDate($old_value), renderDate($new_value));
                } elseif ($field_type == 'FieldDatetime') {
