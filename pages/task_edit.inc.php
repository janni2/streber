...
function taskEdit($task = null)
{
    ...
    if (!$task) {
        ...
        if (!$ids) {
            ...
            exit();
        } elseif (!$task = Task::getEditableById($ids[0])) {
    ...
    ### set up page and write header ####
    {
        ...
        if ($task->id) {
            ...
            } elseif ($task->category == TCATEGORY_FOLDER) {
        ...
        ### task attributes ###
        {
            ...
            if (!$task->isMilestoneOrVersion()) {
                if ($project->settings & PROJECT_SETTING_ENABLE_MILESTONES) {
                    $tab->add(new Form_DropdownGrouped(
                        'task_for_milestone',
                        __('For Milestone'),
...
function taskEditSubmit()
{
    ...
    */
    ...
    if ($parent_task) {
        ...
        } elseif ($parent_task->category != TCATEGORY_FOLDER) {
    ...
    } elseif ($task->issue_report != get('task_issue_report')) {
    ...
    if ($task->category == TCATEGORY_BUG || (isset($task_issue_report) && $task_issue_report)) {
        ...
        ### get from database ###
        elseif ($issue = Issue::getById($task_issue_report)) {
            ### querry form-information ###
            foreach ($issue->fields as $f) {
                $name = $f->name;
                $f->parseForm($issue);
    ...
    if ($task->id == 0) {
        ...
        new FeedbackMessage(
            sprintf(
                __('Created %s %s with ID %s', 'Created <type> <name> with ID <id>...'),
                $task->getLabel(),
                $task->getLink(false),
                $task->id
            )
