...
function TaskView()
{
    ...
    if ($task = Task::getEditableById($tsk)) {
        $editable = true;
    } elseif (!$task = Task::getVisibleById($tsk)) {
    ...
    ### set up page and write header ####
    {
        ...
        if ($project->isPersonVisibleTeamMember($auth->cur_user)) {
            ...
            ### milestone ###
            elseif ($task->isMilestoneOrVersion()) {
                $page->add_function(new PageFunctionGroup([
                    'name' => __('new'),
                ]));
    ...
    #--- list comments --------
    {
        ...
        echo new CommentsOnItemBlock($task);
    }
...
class Block_task_quickedit extends PageBlock
{
    ...
    public function render_quickedit($task)
    {
        ...
        if (Task::getEditableById($task->id)) {
            $editable = true;
        } elseif (!Task::getVisibleById($task->id)) {
            return false;
        }
...
function taskViewAsDocu()
{
    ...
    #--- list files --------
    {
        ...
        echo new FilesAttachedToItemBlock($task);
    }
