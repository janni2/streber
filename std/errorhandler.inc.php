...
function customHandler($number, $error_string, $file, $line, $context = [])
{
    ...
    } elseif (function_exists('confGet') && confGet('DISPLAY_ERROR_LIST') == 'DETAILS') {
    ...
    ### exit php?
    if ($stop == true) {
        echo '<h1>A fatal error occured</h1>';
        if (confGet('DISPLAY_ERROR_LIST')) {
            echo "<pre>$error_string</pre>";
        }
        echo '<p>Sorry, but ' . confGet('APP_NAME') . ' aborted.</p>';
        echo "<p>If you are the administrator of this installation, <br>please help use by sending the errors.log.php file to<br> <a href='http://www.streber-pm.org/phpBB2/viewforum.php?f=3'>www.streber-pm.org</a></p>";
        die();
    }
}
