...
class ChangeLine extends BaseObject
{
    ...
    public const NEW_TOPIC = 1;
    public const NEW_TASK = 2;
    public const NEW_FILE = 3;
    public const UPDATED = 4;
    public const COMMENTED = 5;
    public const MOVED = 6;
    public const RENAMED = 7;
    public const EDITED_WIKI = 8;
    public const ATTACHED_FILE = 9;
    public const ASSIGNED = 10;
    public const DELETED = 11;
    ...
    public static function getChangeLinesForPerson(&$person, $project = null, $date_compare = null)
    {
        global $PH;
        $query_options = [];
    ...
    public static function getChangeLines($query_options)
    {
        global $PH;
                        ...
                        $html_functions = false; # this is to be added after the details
                        {
                            if ($changed_fields_list = ItemChange::getItemChanges([
                                ...
                                if (isset($changed_fields_hash['status'])) {
                                    ...
                                    } elseif ($task->status == STATUS_APPROVED && $task->status > $status_old) {
                                        $txt_what = $html_what = __('approved');
                                        unset($changed_fields_hash['status']);
                                    } elseif ($task->status == STATUS_CLOSED && $task->status > $status_old) {
                                        $txt_what = $html_what = __('closed');
                                        unset($changed_fields_hash['status']);
                                    } elseif ($task->status == STATUS_OPEN && $task->status < $status_old) {
                                        $txt_what = $html_what = __('reopened');
                                        unset($changed_fields_hash['status']);
                                    } elseif ($task->status == STATUS_OPEN) {
                                        unset($changed_fields_hash['status']);
                                    } elseif ($task->status == STATUS_BLOCKED) {
                            ...
                            } elseif (count($changed_fields_hash) == 1 && isset($changed_fields_hash['name'])) {
                                $txt_what = $html_what = __('renamed');
                                $type = ChangeLine::RENAMED;
                            } elseif (count($changed_fields_hash) == 1 && isset($changed_fields_hash['description'])) {
                                $txt_what = $html_what = __('edit wiki');
                                $type = ChangeLine::EDITED_WIKI;
                            } elseif (count($changed_fields_hash)) {                        # status does not count
