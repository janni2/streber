...
abstract class DbItem
{
    ...
    */
    public function getShort($length = false)
    {
    ...
    */
    public function getShortWithTitle()
    {
        if (isset($this->short) && $this->short && $this->short != '') {
    ...
    public static function getItemState($id)
    {
        $id = intval($id);
        $prefix = confGet('DB_TABLE_PREFIX');
        $dbh = new DB_Mysql();
...
class DbProjectItem extends DbItem
{
    ...
    */
    public function __construct($id_or_array = null)
    {
        ...
        */ elseif (is_int($id_or_array) || (is_string($id_or_array) and $id_or_array !== '0')) {
    ...
    public static function initItemFields()
    {
        global $g_item_fields;
        $g_item_fields = [];
    ...
    */
    public static function getVisibleById($id)
    {
        if ($i = DbProjectItem::getById(intval($id))) {
            if ($i->type == ITEM_PROJECT) {
                ...
                */
                return $i;
            } elseif ($i->type == ITEM_PERSON) {
                if ($p = Person::getVisibleById($id)) {
                    return $i;
                } else {
                    return null;
                }
            } elseif ($i->type == ITEM_COMPANY) {
    ...
    public function isEditable()
    {
        ...
        } elseif ($this->type == ITEM_PERSON) {
            return $this->isEditable();
        } elseif ($p = Project::getById($this->project)) {
            if ($p->validateEditItem($this, false)) { # do not abort on error
                return true;
            }
        } elseif ($this->type != ITEM_ISSUE) {
    ...
    */
    ...
    public function validateEditRequestTime($abort_on_failure = true)
    {
        global $PH;
        global $auth;
    ...
    */
    ...
    public function getFieldsChangedForCurrentUser()
    {
        ...
        ### has user seen item? ###
        elseif ($item_people = ItemPerson::getAll([
            'person' => $auth->cur_user->id,
            'item' => $this->id,
        ])) {
            $ip = $item_people[0];
