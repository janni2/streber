...
class Task extends DbProjectItem
{
    ...
    */
    public function getSubtasks($order_by = null)
    {
        if (!$project = Project::getById($this->project)) {
    ...
    public function getSubtasksRecursive()
    {
        if (!$project = Project::getById($this->project)) {
            $PH->abortWarning(__('task without project?'), ERROR_BUG);
        }
    ...
    public function getSubtasksRecursiveAll($options)
    {
        $tasks = [];
    ...
    public function getMilestoneTasks($order_by = null)
    {
        if (!$project = Project::getById($this->project)) {
            $PH->abortWarning(__('task without project?'), ERROR_BUG);
        }
    ...
    */
    public function getNumSubtasks()
    {
        global $auth;
    ...
    public function getFolder()
    {
        ...
        while ($cur_task && intval($cur_task->parent_task)) {
            if (isset($folder_ids[$cur_task->parent_task])) {
                ...
                break;
            } elseif ($cur_task = Task::getVisibleById(intval($cur_task->parent_task))) {
    ...
    */
    public function getFolderLinks($shortnames = true, $project = null)
    {
    ...
    public function getLatestComment($args = [])
    {
        if ($project = Project::getVisibleById($this->project)) {
            $args['on_task'] = $this->id;
    ...
    */
    ...
    public function getSumTaskEfforts()
    {
        global $auth;
        $sum = 0;
    ...
    */
    public function getMilestoneTasksSummary()
    ...
    * getAssignments
    */
    public function isAssignedToPerson($person)
    {
    ...
    * getAssignedPeople
    */
    public function getAssignedPeople($visible_only = true)
    {
        global $auth;
    ...
    */
    ...
    public function getLabel()
    {
        if ($this->is_folder) {
            return __('Folder');
        } elseif ($this->category == TCATEGORY_MILESTONE) {
            return __('Milestone');
        } elseif ($this->category == TCATEGORY_VERSION) {
            return __('Released Milestone');
        } elseif ($this->category == TCATEGORY_DOCU) {
            return __('Topic');
        } elseif ($this->category == TCATEGORY_BUG) {
            return __('Bug');
        } elseif ($this->label) {
            if (!$project = Project::getById($this->project)) {
