...
v1.7.8 GPX, GeoRSS, BASE, KML, PHP support by Barry Hunter

PHP 5, GMT, UTF-8 Modifications by Spreadshirt
www.spreadshirt.net
...
while ($data = $sql_obj->fetchArray()) {
    ...
    $item->enclosure->url='http://www.dailyphp.net/media/voice.mp3';
...
// MBOX, OPML, ATOM, ATOM10, ATOM0.3, HTML, JS, PHP
...
class FeedItem extends HtmlDescribable {
	...
	public $thumb;			###Spread: added declaration to prevent error
...
class FeedCreator extends HtmlDescribable {
	...
	public $format;			###Spread: added declaration to prevent error
	...
	*/
	 ...
	 * @since 1.6.1
	 **/
	public $encoding = "utf-8";				###Spread: default encoding set to utf-8
	 ...
	 */
	static function iTrunc($string, $length) {			###Spread: set to static
...
class FeedDate {
	...
	function rfc822() {
		...
		$date = gmdate("D, d M Y H:i:s T", $this->unix);					###Spread
		#if (TIME_ZONE!="") $date .= " ".str_replace(":","",TIME_ZONE);		###Spread
		return $date;
	}
...
class RSSCreator091 extends FeedCreator {
	...
	function RSSCreator091() {
		...
		$this->encoding = "utf-8";						###Spread: set utf-8 encoding
	}
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			*/
			...
			$itemDate = new FeedDate($this->items[$i]->date); ###Spread
				 $feed.= "            <pubDate>".htmlspecialchars($itemDate->rfc822())."</pubDate>\n"; ###Spread
				# $feed.= "            <pubDate>GMT ".htmlspecialchars(gmdate("Y-m-d H:i:s", $this->items[$i]->date))."</pubDate>\n";	###Spread: always store modified timestamp as GMT		
			}
